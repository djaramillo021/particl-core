...
{
    // Create a message-only window to intercept WM_CLOSE events from efind
 ...
 */
static const char* BITCOIN_PID_FILENAME = "efin.pid";
...
{
#ifdef WIN32
    // Only efind will create a hidden window to receive messages
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-rpccorsdomain=<domain>", "Allow JSON-RPC connections from specified domain (e.g. http://localhost:4200 or \"*\"). This needs to be set if you are using the Efin GUI in a browser.", false, OptionsCategory::RPC);
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<https://efin.io/>";
...
{
    fEfinMode = !gArgs.GetBoolArg("-btcmode", false); // qa tests
    if (!fEfinMode) {
