...
static const uint8_t EFIN_BLOCK_VERSION = 0xA0;
static const uint8_t EFIN_TXN_VERSION = 0xA0;
static const uint8_t MAX_EFIN_TXN_VERSION = 0xBF;
...
inline bool IsEfinTxVersion(int nVersion)
{
    return (nVersion & 0xFF) >= EFIN_TXN_VERSION;
}
...
inline void SerializeTransaction(const TxType& tx, Stream& s) {
    ...
    if (IsEfinTxVersion(tx.nVersion)) {
...
public:
    ...
    static const int32_t CURRENT_EFIN_VERSION=0xA0;
    ...
    static const int32_t MAX_STANDARD_EFIN_VERSION=0xA0;
...
public:
    ...
    bool IsEfinVersion() const {
        return IsEfinTxVersion(nVersion);
    }
    ...
    {
        return IsEfinTxVersion(nVersion) ? vpout.size() : vout.size();
    }
