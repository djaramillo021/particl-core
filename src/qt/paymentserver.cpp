...
const QString BITCOIN_IPC_PREFIX("efin:");
#ifdef ENABLE_BIP70
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start efin: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("efin://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'efin://' is not a valid URI. Use 'efin:' instead."),
