...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent, bool allow_stakeonly)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Efin address (e.g. %1)").arg(
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("efin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Efin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Efin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Efin (%s).lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Efin\n";
        else
            optionFile << strprintf("Name=Efin (%s)\n", chain);
