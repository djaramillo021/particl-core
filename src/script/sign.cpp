...
public:
    ...
    bool is_efin_tx = false;
    bool IsEfinVersion() const override { return is_efin_tx; }
};
...
SignatureData DataFromTransaction(const CMutableTransaction& tx, unsigned int nIn, const std::vector<uint8_t> &amount, const CScript &scriptPubKey)
{
    ...
    extractor_checker.is_efin_tx = tx.IsEfinVersion();
...
class DummySignatureCheckerEfin : public DummySignatureChecker
{
// IsEfinVersion() must return true to skip stack evaluation
public:
    DummySignatureCheckerEfin() : DummySignatureChecker() {}
    bool IsEfinVersion() const override { return true; }
};
const DummySignatureCheckerEfin DUMMY_CHECKER_EFIN;

class DummySignatureCreatorEfin : public DummySignatureCreator {
public:
    DummySignatureCreatorEfin() : DummySignatureCreator(33, 32) {}
    const BaseSignatureChecker& Checker() const override { return DUMMY_CHECKER_EFIN; }
    bool IsEfinVersion() const override { return true; }
};
...
const BaseSignatureCreator& DUMMY_SIGNATURE_CREATOR_EFIN = DummySignatureCreatorEfin();
