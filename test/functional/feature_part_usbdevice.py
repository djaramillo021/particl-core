...
from test_framework.test_efin import (
    EfinTestFramework,
    isclose,
    getIndexAtProperty,
)
...
class USBDeviceTest(EfinTestFramework):
    ...
    def run_test(self):
        ...
        # Check that efin has been built with USB device enabled
        ...
        if not config["components"].getboolean("ENABLE_USBDEVICE"):
            raise SkipTest("efind has not been built with usb device enabled.")
